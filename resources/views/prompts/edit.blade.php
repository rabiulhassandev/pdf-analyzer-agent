        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <div class="relative">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="searchInput" class="w-full pl-9 pr-3 py-2 bg-slate-50 border-slate-200 border rounded-lg text-sm focus:border-blue-500 outline-none transition-colors" placeholder="By name..." value="{{ request('search') }}">
                </div>
            </div>
            <div>
                <label class="text-xs font-semibold text-slate-500 mb-1.5 block uppercase tracking-wider">Type</label>
                <select id="typeFilter" class="w-full px-3 py-2 bg-slate-50 border-slate-200 border rounded-lg text-sm focus:border-blue-500 outline-none transition-colors">
            ...
            <div>
                ...
                <select id="statusFilter" class="w-full px-3 py-2 bg-slate-50 border-slate-200 border rounded-lg text-sm focus:border-blue-500 outline-none transition-colors">
            ...
            <div class="flex items-end pb-0.5">
                <a href="{{ route('prompts.index') }}" class="text-blue-600 font-semibold text-sm hover:underline px-2">Clear All Filters</a>
            </div>
        </div>
                ...
                <div class="bg-white border border-slate-200 rounded-xl shadow-[0_4px_20px_rgba(15,23,42,0.05)] overflow-hidden hover:shadow-lg transition-shadow group">
                    <div class="p-5 border-b border-slate-100 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span class="material-symbols-outlined {{ $prompt->type === 'entity_extraction' ? 'text-blue-600' : ($prompt->type === 'risk_assessment' ? 'text-amber-600' : ($prompt->type === 'summary' ? 'text-emerald-600' : 'text-purple-600')) }}">auto_awesome</span>
                            <span class="text-[20px] font-semibold text-slate-900">{{ $prompt->name }}</span>
                        </div>
                        <span class="px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide {{ $prompt->status === 'active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : ($prompt->status === 'draft' ? 'bg-slate-100 text-slate-600' : 'bg-slate-200 text-slate-500') }}">
                            {{ $prompt->status }}
                        </span>
                    </div>
                    <div class="p-5">
                        ...
                        <div class="flex flex-wrap gap-2 mb-4">
                            <span class="px-2 py-1 bg-blue-50 text-blue-700 text-[10px] font-medium rounded">{{ $prompt->model }}</span>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-slate-100">
                            ...
                            @if ($prompt->accuracy_score)
                                <div class="flex items-center gap-1 text-xs font-medium {{ $prompt->accuracy_score >= 90 ? 'text-emerald-600' : 'text-amber-600' }}">
                    ...
                    <div class="px-5 py-3 bg-slate-50 border-t border-slate-100 flex gap-2">
                        ...
                        <a href="{{ route('prompts.edit', $prompt) }}" class="flex-1 text-center py-2 text-sm font-medium text-blue-600 hover:bg-blue-50 rounded transition-colors">Edit</a>
                    </div>
                </div>
            @empty
                <div class="md:col-span-2 lg:col-span-3 bg-white border border-slate-200 rounded-xl p-12 text-center">
                    ...
                    <a href="{{ route('prompts.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700">
                        Create First Prompt
                    </a>
                </div>
            @endforelse
